...
def rubberband_baseline(x, y, plot=True) -> np.array:
    """Finds the convex hull of an xy plot, uses the bottom side as the baseline.
    ...
    v = ConvexHull(np.stack([x, y]).T).vertices

    if x[1] > x[0]:  # if increasing order
        v = np.roll(
            v, -v.argmin()
        )  # rotate convex hull vertices until they start from the lowest one
        v = v[: v.argmax()]  # Leave only the ascending part
    else:  # do the opposite if in decreasing order
    ...
    if plot:
        ...
        plt.plot(x, bl, "r:", label="baseline")
        plt.legend()
...
def remove_baseline(spectrum, sensitivity=5):
    ...
    spectrum_0 = np.hstack(
        [np.full((lp,), spectrum[0]), spectrum, np.full((lp,), spectrum[-1])]
    )
    l2 = len(spectrum_0)
...
def __n_lorentzian_generator(n, x, *args):
    ...
    if len(args) != n * 3:
        print(
            "Error: must be three arguments (amplitude, center, and width) per lorentzian!"
        )
        return
...
def __n_gaussian_generator(n, x, *args):
    ...
    if len(args) != n * 3:
        print(
            "Error: must be three arguments (amplitude, center, and standard deviation) per gaussian!"
        )
        return
...
def voigt(x, gamplitude, gcenter, gsigma, lamplitude, lcenter, lwidth):
    return (
        gamplitude
        * (1 / (gsigma * (np.sqrt(2 * np.pi))))
        * (np.exp(-((x - gcenter) ** 2) / ((2 * gsigma) ** 2)))
    ) + (lamplitude * lwidth ** 2 / ((x - lcenter) ** 2 + lwidth ** 2))
...
def __n_voigt_generator(n, x, *args):
    ...
    if len(args) != n * 6:
        print(
            "Error: must be six arguments (gaussian amplitude, center, and standard deviation, lorentzian amplitude,center,andwidth) per voigt!"
        )
        return
